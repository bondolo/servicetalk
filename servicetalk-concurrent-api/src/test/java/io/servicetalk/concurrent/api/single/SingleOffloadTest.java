...
public class SingleOffloadTest {
    ...
    static class VerifySubscriber implements SingleSource.Subscriber<String> {

        final AtomicReference<Cancellable> canceller = new AtomicReference<>();
        final AtomicBoolean subscribed = new AtomicBoolean();
        final AtomicBoolean complete = new AtomicBoolean();

        final Predicate<? super String> itemMatches;
        final Predicate<Throwable> errorMatches;
        final Matcher<? super Thread> subscribeThread;
        final Matcher<? super Thread> publishThread;

        VerifySubscriber(@Nullable Predicate<? super String> itemMatches, @Nullable Predicate<Throwable> errorMatches,
    ...
    private enum Result {
        ...
        Result(Supplier<String> supplier) {
    ...
    public void singleOffloadTest(final Result result,
                                  ...
                                  final OffloadPublish publishOffload) {
        Single<String> single = Single.fromSupplier(result.supplier);
        SingleSource<String> source = toSource(OffloadSubscribe.OFFLOAD_SUBSCRIBE == subscribeOffload ?
                OffloadPublish.OFFLOAD_PUBLISH == publishOffload ?
                        single.publishAndSubscribeOn(executor) : single.subscribeOn(executor)
                : OffloadPublish.OFFLOAD_PUBLISH == publishOffload ?
                        single.publishOn(executor) : single.publishAndSubscribeOn(immediate()));
